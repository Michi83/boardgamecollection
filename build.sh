inkscape img/svg/blackbishop.svg --export-type=png --export-filename=img/png/blackbishop.png
inkscape img/svg/blackcannon.svg --export-type=png --export-filename=img/png/blackcannon.png
inkscape img/svg/blackghostpiece.svg --export-type=png --export-filename=img/png/blackghostpiece.png
inkscape img/svg/blackking.svg --export-type=png --export-filename=img/png/blackking.png
inkscape img/svg/blackknight.svg --export-type=png --export-filename=img/png/blackknight.png
inkscape img/svg/blackpawn.svg --export-type=png --export-filename=img/png/blackpawn.png
inkscape img/svg/blackpiece.svg --export-type=png --export-filename=img/png/blackpiece.png
inkscape img/svg/blackpromotedbishop.svg --export-type=png --export-filename=img/png/blackpromotedbishop.png
inkscape img/svg/blackpromotedknight.svg --export-type=png --export-filename=img/png/blackpromotedknight.png
inkscape img/svg/blackpromotedlance.svg --export-type=png --export-filename=img/png/blackpromotedlance.png
inkscape img/svg/blackpromotedpawn.svg --export-type=png --export-filename=img/png/blackpromotedpawn.png
inkscape img/svg/blackpromotedrook.svg --export-type=png --export-filename=img/png/blackpromotedrook.png
inkscape img/svg/blackpromotedsilver.svg --export-type=png --export-filename=img/png/blackpromotedsilver.png
inkscape img/svg/blackqueen.svg --export-type=png --export-filename=img/png/blackqueen.png
inkscape img/svg/blackrook.svg --export-type=png --export-filename=img/png/blackrook.png
inkscape img/svg/blackshogibishop.svg --export-type=png --export-filename=img/png/blackshogibishop.png
inkscape img/svg/blackshogigold.svg --export-type=png --export-filename=img/png/blackshogigold.png
inkscape img/svg/blackshogiking.svg --export-type=png --export-filename=img/png/blackshogiking.png
inkscape img/svg/blackshogiknight.svg --export-type=png --export-filename=img/png/blackshogiknight.png
inkscape img/svg/blackshogilance.svg --export-type=png --export-filename=img/png/blackshogilance.png
inkscape img/svg/blackshogipawn.svg --export-type=png --export-filename=img/png/blackshogipawn.png
inkscape img/svg/blackshogirook.svg --export-type=png --export-filename=img/png/blackshogirook.png
inkscape img/svg/blackshogisilver.svg --export-type=png --export-filename=img/png/blackshogisilver.png
inkscape img/svg/blacksmallghostpiece.svg --export-type=png --export-filename=img/png/blacksmallghostpiece.png
inkscape img/svg/blacksmallking.svg --export-type=png --export-filename=img/png/blacksmallking.png
inkscape img/svg/blacksmallpiece.svg --export-type=png --export-filename=img/png/blacksmallpiece.png
inkscape img/svg/chess.svg --export-type=png --export-filename=img/png/chess.png
inkscape img/svg/chess10x10.svg --export-type=png --export-filename=img/png/chess10x10.png
inkscape img/svg/go9.svg --export-type=png --export-filename=img/png/go9.png
inkscape img/svg/go13.svg --export-type=png --export-filename=img/png/go13.png
inkscape img/svg/go19.svg --export-type=png --export-filename=img/png/go19.png
inkscape img/svg/gomoku.svg --export-type=png --export-filename=img/png/gomoku.png
inkscape img/svg/mancala.svg --export-type=png --export-filename=img/png/mancala.png
inkscape img/svg/morris.svg --export-type=png --export-filename=img/png/morris.png
inkscape img/svg/neutralsmallpiece.svg --export-type=png --export-filename=img/png/neutralsmallpiece.png
inkscape img/svg/reversi.svg --export-type=png --export-filename=img/png/reversi.png
inkscape img/svg/selection.svg --export-type=png --export-filename=img/png/selection.png
inkscape img/svg/shogi.svg --export-type=png --export-filename=img/png/shogi.png
inkscape img/svg/tablut.svg --export-type=png --export-filename=img/png/tablut.png
inkscape img/svg/whitebishop.svg --export-type=png --export-filename=img/png/whitebishop.png
inkscape img/svg/whitecannon.svg --export-type=png --export-filename=img/png/whitecannon.png
inkscape img/svg/whiteghostpiece.svg --export-type=png --export-filename=img/png/whiteghostpiece.png
inkscape img/svg/whiteking.svg --export-type=png --export-filename=img/png/whiteking.png
inkscape img/svg/whiteknight.svg --export-type=png --export-filename=img/png/whiteknight.png
inkscape img/svg/whitepawn.svg --export-type=png --export-filename=img/png/whitepawn.png
inkscape img/svg/whitepiece.svg --export-type=png --export-filename=img/png/whitepiece.png
inkscape img/svg/whitepromotedbishop.svg --export-type=png --export-filename=img/png/whitepromotedbishop.png
inkscape img/svg/whitepromotedknight.svg --export-type=png --export-filename=img/png/whitepromotedknight.png
inkscape img/svg/whitepromotedlance.svg --export-type=png --export-filename=img/png/whitepromotedlance.png
inkscape img/svg/whitepromotedpawn.svg --export-type=png --export-filename=img/png/whitepromotedpawn.png
inkscape img/svg/whitepromotedrook.svg --export-type=png --export-filename=img/png/whitepromotedrook.png
inkscape img/svg/whitepromotedsilver.svg --export-type=png --export-filename=img/png/whitepromotedsilver.png
inkscape img/svg/whitequeen.svg --export-type=png --export-filename=img/png/whitequeen.png
inkscape img/svg/whiterook.svg --export-type=png --export-filename=img/png/whiterook.png
inkscape img/svg/whiteshogibishop.svg --export-type=png --export-filename=img/png/whiteshogibishop.png
inkscape img/svg/whiteshogigold.svg --export-type=png --export-filename=img/png/whiteshogigold.png
inkscape img/svg/whiteshogiking.svg --export-type=png --export-filename=img/png/whiteshogiking.png
inkscape img/svg/whiteshogiknight.svg --export-type=png --export-filename=img/png/whiteshogiknight.png
inkscape img/svg/whiteshogilance.svg --export-type=png --export-filename=img/png/whiteshogilance.png
inkscape img/svg/whiteshogipawn.svg --export-type=png --export-filename=img/png/whiteshogipawn.png
inkscape img/svg/whiteshogirook.svg --export-type=png --export-filename=img/png/whiteshogirook.png
inkscape img/svg/whiteshogisilver.svg --export-type=png --export-filename=img/png/whiteshogisilver.png
inkscape img/svg/whitesmallghostpiece.svg --export-type=png --export-filename=img/png/whitesmallghostpiece.png
inkscape img/svg/whitesmallking.svg --export-type=png --export-filename=img/png/whitesmallking.png
inkscape img/svg/whitesmallpiece.svg --export-type=png --export-filename=img/png/whitesmallpiece.png
inkscape img/svg/xiangqi.svg --export-type=png --export-filename=img/png/xiangqi.png
mkdir bin
javac -d bin -sourcepath src src/CheckersState.java
javac -d bin -sourcepath src src/ChessState.java
javac -d bin -sourcepath src src/Go9State.java
javac -d bin -sourcepath src src/Go13State.java
javac -d bin -sourcepath src src/Go19State.java
javac -d bin -sourcepath src src/GomokuState.java
javac -d bin -sourcepath src src/InternationalCheckersState.java
javac -d bin -sourcepath src src/MainWindow.java
javac -d bin -sourcepath src src/MorrisState.java
javac -d bin -sourcepath src src/OwareState.java
javac -d bin -sourcepath src src/ReversiState.java
javac -d bin -sourcepath src src/RussianCheckersState.java
javac -d bin -sourcepath src src/ShogiState.java
javac -d bin -sourcepath src src/TablutState.java
javac -d bin -sourcepath src src/XiangqiState.java
jar -c -e MainWindow -f "Board Game Collection.jar" -C bin .
rm -rf bin